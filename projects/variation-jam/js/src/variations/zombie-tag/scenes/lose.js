...
export default class LoseScene extends BaseScene {
  static key = "zombie-tag-game.lose";
  static label = "Zombie Tag Game: Lose Scene";
  ...
  setup(p5) {
    ...
    SoundBook.load(
      p5,
      "horror-ambiance",
      "assets/sounds/Tense Horror Ambience (Fast Violin Strike Noise) - Sound Effect for editing Youtube-rLtH4Uzrx_4.mp3"
    ).then((soundFile) => {
  ...
  draw(p5) {
    ...
    p5.push();
    {
      ...
      p5.text("YOU DIED.", p5.width / 2, p5.height / 2);
    }
    p5.pop();
    ...
    if (secondsLeft === 0) {
      ...
      if (
        !SoundBook.isSentinel(this.soundEffect) &&
        this.soundEffect !== null
      ) {
